package com.hmeclazcke.jobsearchplatform.internaljobs.application.service;
...
import com.hmeclazcke.jobsearchplatform.internaljobs.application.port.in.usecase.ProcessSearchRequestUseCase;
...
import com.hmeclazcke.jobsearchplatform.internaljobs.application.port.out.publisher.PublishProviderFailedPort;
import com.hmeclazcke.jobsearchplatform.internaljobs.application.port.out.publisher.PublishProviderResultsPort;
...
public class ProcessSearchRequestService implements ProcessSearchRequestUseCase {
    ...
    private final SearchInternalJobsPort searchInternalJobsPort;
    ...
    public void process(ProcessSearchRequestCommand command) {
        try {
            List<JobDto> jobs = searchInternalJobsPort.searchJobs(command.criteria());
            ...
            log.info("Published internal jobs results: searchId={}, jobsCount={}",
                    command.searchId(),
                    jobs.size());
            ...
            log.warn("Published internal jobs failure: searchId={}, failureType={}, message={}",
                    command.searchId(),
                    exception.failureType(),
                    exception.getMessage());
        }
